...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：1. ：报告标题，针对兴业证券数据库应用redo卷时延增高问题的分析文档。
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：2. ：文档，包括问题概述、分析、数据分析和结论等章节结构。
3. 问题概述：两个数据库应用自3月10日起redo日志卷写时延增高，多次出现30秒以上延迟。
4. 问题分析：对时延增高问题进行分析，涵盖存储日志和性能数据两大方面。
5. 存储 Dump 日志分析：分析存储Dump日志，从MDKC和RDKC存储日志中查找延迟原因。
6. 性能数据分析：通过存储资源利用率、redo日志卷性能及读写响应时间进行性能分析。
7. 结论和建议：给出问题的最终结论和改进建议，待后续章节展开。
逻辑关联：包含小节：存储 Dump 日志分析 . 6
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：8. ：同第3章，描述两套数据库应用redo日志卷自3月10日起时延增高的事件。
逻辑关联：上一节：结论和建议... .21
下一节：2 问题分析
---
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：9. ：同第4章，对时延问题进行系统分析，拆分为日志和性能分析。
逻辑关联：上一节：1 问题概述
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：10. ：同第5章，聚焦存储Dump日志，分MDKC和RDKC分析。
逻辑关联：上一节：2 问题分析
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：11. ：分析MDKC存储日志，记录WRITE操作时延事件。
逻辑关联：上一节：2.1 存储 Dump 日志分析
下一节：WRITE
---
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：12. ：MDKC日志中操作的时延记录，显示多次30秒左右延迟。
逻辑关联：上一节：2.1.1 应用对应的 MDKC存储日志分析
...
. Ona WRITE CMD Sequence,such as shown above,depending on which FRAME gets lost of damaged,wil cause different SSB to be logged and cause a different level of Response Impact to the Host.
...
The "Round-Trip Gaps 1&2" shown above depend entirely on the DISTANCE between the HBA/INl and Target PORTS. Rule of thumb is “1ms" per“100KM" per Round-Trip.This holds well pretty wellif the LINKS are purely DARK-FIBRE LINKS. If there is FCIP in a WAN things can change dramatically to the worse.

# 2.1.2 应用对应的 RDKC存储日志分析

---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：13. ：分析RDKC存储日志，关注READ操作延迟。
逻辑关联：上一节：WRITE
下一节：READ
---
...
| 00000000 | 0 |Response Queue Full
| 00000000 | 0 |ATIO Queue Full
| 000000O<fcel>| 79259447 |Transmit Frame Count from 16G_CHA Protocol CHIP |
| DF924962 | 37599621<fcel>|Received Frame Count From 16G_CHA Protocol CHIP |
| 1888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：14. ：RDKC日志中操作的时延情况，待具体数据补充。
逻辑关联：上一节：2.1.2 应用对应的 RDKC存储日志分析
下一节：2.2 性能数据分析
---
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：15. ：同第6章，从资源利用率、redo卷性能及响应时间分析。
逻辑关联：上一节：READ
下一节：2.2.1 存储资源利用率分析
---
...
From:2025/3/128:00:00to2025/3/1211:00:00(samplingrate:1min)
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：17. ：专门对redo日志卷性能进行分析，查找延迟根源。
逻辑关联：上一节：2.2.1 存储资源利用率分析
...
LU Transfer 17(ZYO32\_DB01JQP) "SN:31028(VSP 5100,5500,5100H,5500H)"
...
| ---------- | ------------------ |
| 2025/3/12 8:01:00 | 0 |
| 2025/3/12 8:30:00 | 42 |
| 2025/3/12 8:51:00 | 37 |
| 2025/3/12 9:01:00 | 34 |
| 2025/3/12 9:30:00 | 33 |
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：18. ：研究存储读写响应时间对redo写时延的影响。
逻辑关联：上一节：2.2.2 Redo 日志卷性能分析
下一节：3 结论和建议
---
...
</details> 写 I/O：延时可能出现在 XRDY 状态帧传输至主机时所经过的链路以及和主机确认写完成信号时所经过的链路
Write Sequence :
...
---
文档概要：该报告分析了兴业证券两套数据库应用（UF20-DB01BHP、ZYO32_DB01JQ）自2025年3月10日起redo日志卷时延增高的问题，事件显示写时延达11-30秒，涉及福州滨海与上海金桥的日立GAD双活存储，并进行了存储日志与性能分析。
本段概要：19. ：同第7章，总结问题原因并给出后续改进建议。
逻辑关联：上一节：2.2.3 关于存储读写响应时间
---
